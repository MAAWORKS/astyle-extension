...
namespace AStyleExtension
{
    public partial class AStyleGeneralOptionsControl : UserControl
    {
        ...
        private void OnButtonCPPSettingsClick(object sender, EventArgs e)
        {
            ...
            form.SetControls(this.CppOptions);
        ...
        private void OnButtonCSEditClick(object sender, EventArgs e)
        {
            ...
            form.SetCommandLine(this.CsOptions);
        ...
        private void OnButtonExportClick(object sender, EventArgs e)
        {
            ...
            {
                CppCommandLine = this.CppOptions,
                CsCommandLine = this.CsOptions,
                CppFormatOnSave = this.CppFormatOnSave,
                CsFormatOnSave = this.CsFormatOnSave,
                Version = System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString()
            };

            // ReSharper disable once UseStringInterpolation
            var filePath = string.Format("{0}\\AStyleExtension-{1}-{2}-{3}.cfg", folderBrowserDialog.SelectedPath, DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
            ...
            try
            {
                ...
                textBoxDetails.Text = $"Your settings were successfully exported to {filePath}.";
            }
            catch (Exception)
            {
                textBoxDetails.Text = $"Failed to export settings to {filePath}.";
            }
        }
        ...
        private void OnButtonImportClick(object sender, EventArgs e)
        {
            ...
            using (var fs = new FileStream(openFileDialog.FileName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                {
                    textBoxDetails.Text = $"Failed to import settings from {openFileDialog.FileName}.";
                }
            }
            ...
            this.CppOptions = settings.CppCommandLine;
            checkBoxCppFormatOnSave.Checked = settings.CppFormatOnSave;

            if (this.IsCSarpEnabled)
            {
                this.CsOptions = settings.CsCommandLine;
                checkBoxCsFormatOnSave.Checked = settings.CsFormatOnSave;
            }


            textBoxDetails.Text = $"Your settings were successfully imported from {openFileDialog.FileName}.{Environment.NewLine}Click OK to save the changes.";
        }
    }
}